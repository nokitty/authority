...
namespace DBC
{
    public class PublicPerson:DBCBase
    {
        ...
        public PublicPerson(int id):base(DBTables.PublicPerson)
        {
            Initialize("id=?", id);
        }
        ...
        {
            var sql = "select id,casecode,age,sexy,courtname,areaname,gistid,regdate,gistunit,duty,performance,disrupttypename,publishdate,personid from " + _tableName + " where " + filter;
