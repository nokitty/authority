...
//					url              : "/upload/UploadAction",  	// 上传文件的路径
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
					...
					html += '			</div>';
					html += '			<div id="preview" class="upload_preview clearfix" style="padding-right:10px;padding-bottom:10px;">'+para.upLoadedHtml+'<div class="add-img-items webuploader_pick"><img src="images/addImg.png"></div></div>';
					...
					html += '		</div>';
					html += '		<div id="uploadInf" class="upload_inf" style="display:none;">'+para.upLoadedUrl+'</div>';
					html += '	</div>';
					html += '</form>';
				}else{
					var imgWidth = parseInt(para.itemWidth.replace("px", ""))-10;
				    ...
				    html += '							<img class="upload_image" src="images/add_img.png" />';
				    html += '						</div>';
				    html += '					</a>';
				    html += '				</div>';
					html += '			</div>';
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				if (file.type.indexOf("image") == 0) {
					...
					html += '<div class="file_bar_wrap"><div class="file_bar file_hover">';
					...
					html += '		</div>';
					html += '	</a>';
					html += '	<div class="file_progress_wrap"><p id="uploadProgress_'+file.index+'" class="file_progress"></p></div>';
					html += '	<div class="file_failure_wrap"><p id="uploadFailure_'+file.index+'" class="file_failure">上传失败，请重试</p></div>';
					html += '	<div class="file_success_wrap"><p id="uploadSuccess_'+file.index+'" class="file_success"></p></div>';
					html += '</div>';
                	
				}else{
					html += '<div id="uploadList_'+ file.index +'" class="upload_append_list">';
					html += '<div class="file_bar_wrap"><div class="file_bar">';
					html += '		<div style="padding-left:5px;">';
					html += '			<p class="file_name">' + file.name + '</p>';
					html += delHtml;   // 删除按钮的html
					html += '		</div>';
				...
				var params = {
					...
					onSelect: function(selectFiles, allFiles) {
						...
						if(ZYFILE.funReturnNeedFiles().length > 0){
							...
							//alert("请先选中文件再点击上传");
							alertErrorMsg("请先选中文件再点击上传!");
						}
					},
					onDelete: function(file, files) {
						// 移除效果
						$("#uploadList_" + file.index).fadeOut();
						$("#uploadInf").find('p[data-file-index='+file.index+']').remove();
						// 重新设置统计栏信息
					...
					onSuccess: function(file, response) {
						...
						$(".file_bar_wrap").show();//隐藏删除按钮
					//	$("#uploadInf").append("<p>上传成功，文件地址是：" + response + "</p>");
						$("#uploadInf").append('<p class="upLoadUrlItems" data-file-index="'+file.index+'">' + response +'</p>');
						// 根据配置参数确定隐不隐藏上传成功的文件
						if(para.finishDel){
							// 移除效果
							$("#uploadList_" + file.index).fadeOut();
