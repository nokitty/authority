...
function subExposureForm(){
	...
	//$("#exposureDataAccessId").val(accessId);
	//$("#reportPersonId").val(reportPersonId);
	...
	//上传完的图片url数据载入input隐藏表单
	var array = new Array();
	$("#uploadInf .upLoadUrlItems").each(function(){
		var url = $(this).text();
		array.push(url);
	});
	array = array.join(",");
	$("#proofImgUrlId").val(array);
	var length = validaSubExposureForm($("#exposureFormId"));
	if(length > 0){
		return;
	}else{
		if(accessId == '' || accessId == null || accessId == undefined){
			...
			if(subFlag == true){
				...
				$.ajax({
					...
					timeout : 30000,
					dataType : "json",
					url : "saveReportedPerson.action",
					data :$("#exposureFormId").serialize(),
					beforeSend : function(){
						showLoading();
					},
					success : function(data){
						...
						if(data.ErrorCode == 0){
							if(data.data.success == 1){
								if(reportPersonAccessId == '' || reportPersonAccessId == null || reportPersonAccessId == "null" || reportPersonAccessId == undefined){
	                               $.cookie("exposureAccessId","true",{path : '/'});
								}else{
									$.cookie("reportPersonId",reportPersonId,{path:'/'});
								}
								$.cookie("reportedPersonId",data.data.reportedPerson.reportedPersonId,{path : '/'});
								location.href = basePath + "lawMobile/exposureDetail.jsp";
							}
						}else if(data.data.success == -1){
							alertErrorMsg("您的举报信息已成功提交，不能重复举报！");
						}else{
