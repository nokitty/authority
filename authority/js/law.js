...
function toSearch(){
	...
	if(length > 0){
		...
		if(accessId == '' || accessId == null || accessId == undefined){
			...
			alertError(accessIdError);

		}else{

			var flag = false;//精确查找标准位--true为精确查找

			var idcard = $("#searchFormId #idcard").val();

			var name = $("#searchFormId #name").val();
			...
			if(flag){
				...
				location.href = basePath + 'lawMobile/lawList.jsp?f=ttzchrf53ekqqbro&n='+name+'&id='+idcard;

			}else{

				$.cookie("wLawName",name,{path : '/'});

				$.cookie("wLawIdcard",'',{path : '/'});

				location.href = basePath + 'lawMobile/wholeLawList.jsp?n='+name+'&id='+idcard;
